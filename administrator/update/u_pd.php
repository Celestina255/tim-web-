...
mysqli_query($con, "DELETE FROM tb_datapdinas WHERE kodepd='$kode'");
 ...
 mysqli_query($con, "UPDATE tb_detailsurat SET no='$no', nik='$nip1', nama='$nama1', detail='$detail' WHERE kode='$kode'");
...
for ($i=0; $i<$jumlah_pilih;$i++){
        mysqli_query($con, "INSERT INTO tb_datapdinas SET
            kodepd      = '$kodepd[$i]',
            nm          = '$nm[$i]',
            tgl_lahir   = '$tgll[$i]',
            ket         = '$ket[$i]'
...
echo "<script>alert('Surat Perjalanan Dinas berhasil diupdate!'); window.location = '../../administrator/index.php?page=c_pdinas&kode=$kode'</script>"; 		
